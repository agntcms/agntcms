...
**The whole section** is replaced through a picker modal. Hovering over a section shows an edit icon (⇄); clicking opens a modal listing the available section types. Selecting a new type calls `POST /api/agntcms/draft/replace-section`, which swaps the section with one of the new type seeded from `sectionDefaults` (derived from the section registry — see §3). The seed is **representative content**: each section ships a single `previewData` sample that the picker renders in its preview card and that is also the insertion seed, so a freshly inserted section is never empty (lists carry real items, copy reads as finished). `previewData` is layered over the auto-generated per-field placeholders, which remain only as the fallback for fields the author left out of the sample. This is a deliberate reversal of the earlier "preview-only, never inserted" rule: there is one authored sample per section, used for both purposes. Existing content in that section is lost — the operation is destructive by design (different schemas, no auto-conversion). Recovery is through draft history.
